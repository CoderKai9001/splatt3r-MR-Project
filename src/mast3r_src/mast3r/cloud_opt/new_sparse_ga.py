...
def sparse_global_alignment(imgs, pairs_in, cache_path, model, subsample=8, desc_conf='desc_conf',
    ...
    tmp_pairs, pairwise_scores, canonical_views, canonical_gaussians, canonical_paths, preds_21 = \
        prepare_canonical_data(imgs, pairs, subsample, cache_path=cache_path, mode='avg-angle', device=device)
    print(f'[CANON_KEYS]{list(canonical_gaussians.keys())}')
    # print(f"[DEBUG] Canonical gaussians extracted for {len(canonical_gaussians)} images and shape {canonical_gaussians[0].shape if canonical_gaussians else 'N/A'}.")
    ...
    final_res = res_fine if res_fine is not None else res_coarse
    
    # 2. Save PLY
    # Ensure you import save_colored_ply
    # save_colored_ply(
    #     output_path= os.path.join(cache_path, "fused_scene.ply"),
    #     canonical_gaussians=canonical_gaussians,
    #     canonical_views=canonical_views,
    #     cam2w=final_res['cam2w'],       # Optimized Poses
    #     intrinsics=final_res['intrinsics'], # Optimized Intrinsics
    #     img_paths=imgs
    # )
...
def save_colored_ply(output_path, canonical_gaussians, canonical_views, cam2w, intrinsics, img_paths):
    """
    Exports the scene to PLY with both Gaussian attributes (for Splat viewers)
    AND standard RGB colors (for MeshLab/CloudCompare).
    """
    print(f"[INFO] Exporting Colored Gaussian Splat to {output_path}...")
    ...
    # 1. Move Global Parameters to CPU
    cam2w = cam2w.detach().cpu()
    intrinsics = intrinsics.detach().cpu()

    for i, img_path in enumerate(img_paths):
        # Check if Gaussians exist for this image
        gaussians = canonical_gaussians.get(img_path)
        if gaussians is None: continue
            
        try:
            # 2. Get Geometry Info
            pp_old, shape, focal_old, core_depth, _, _, _ = canonical_views[img_path]
            
            # Move depth to CPU immediately to match cam2w
            z = core_depth.float().cpu() 
            
            # Upscale depth if needed 
            # (Gaussian map is usually 512x512, depth might be subsampled)
            tgt_H, tgt_W = gaussians['sh'].shape[:2]
            
            if z.shape[:2] != (tgt_H, tgt_W):
                # Interpolate expects (N, C, H, W)
                z = torch.nn.functional.interpolate(z[None, None, ...], size=(tgt_H, tgt_W), mode='bilinear')[0,0]

            # 3. Create Pixel Grid on CPU
            y, x = torch.meshgrid(torch.arange(tgt_H), torch.arange(tgt_W), indexing='ij')
            x = x.float()
            y = y.float()

            # 4. Unproject to Local Camera Space (Pinhole)
            # Use the OPTIMIZED intrinsics
            K = intrinsics[i]
            fx, fy = K[0,0], K[1,1]
            cx, cy = K[0,2], K[1,2]
            ...
            xyz_local = torch.stack((x_local, y_local, z_local), dim=-1).reshape(-1, 3)
            ...
            # (R @ xyz.T).T + t  <-- All tensors are now on CPU!
            xyz_world = (R @ xyz_local.T).T + t
            
            # 6. Filter invalid points (depth too close or too far)
            valid_mask = (z.reshape(-1) > 0.01) & (z.reshape(-1) < 100.0)
            
            # 7. Flatten Attributes & Move to CPU
            # We must ensure gaussian attributes are also on CPU before masking
            flat_scale = gaussians['scales'].detach().cpu().reshape(-1, 3)
            flat_rot = gaussians['rotations'].detach().cpu().reshape(-1, 4)
            flat_sh = gaussians['sh'].detach().cpu().reshape(-1, 3) # RGB [0,1]
            flat_opac = gaussians['opacities'].detach().cpu().reshape(-1, 1)
            
            # Apply Mask
            all_xyz.append(xyz_world[valid_mask])
            all_scales.append(flat_scale[valid_mask])
            all_rots.append(flat_rot[valid_mask])
            all_sh.append(flat_sh[valid_mask])
            all_opacities.append(flat_opac[valid_mask])
            all_rgb.append(flat_sh[valid_mask]) # Store raw RGB for standard PLY color
    ...
    if not all_xyz:
        print("[ERR] No points generated!")
        return

    # Concatenate
    xyz = torch.cat(all_xyz).numpy()
    opacities = torch.cat(all_opacities).numpy()
    scales = torch.cat(all_scales).numpy()
    rots = torch.cat(all_rots).numpy()
    shs = torch.cat(all_sh).numpy()
    rgbs = torch.cat(all_rgb).numpy()

    # --- COLOR CONVERSION ---
    # 1. For Splat Viewers (f_dc): Convert RGB [0,1] -> SH DC
    # Formula: SH = (RGB - 0.5) / 0.28209
    # Ensure RGB is clipped to [0, 1] before conversion
    rgbs = np.clip(rgbs, 0, 1)
    C0 = 0.28209479177387814
    f_dc = (rgbs - 0.5) / C0
    
    # 2. For MeshLab/Standard Viewers (red, green, blue):
    # Convert RGB [0,1] -> [0, 255] uint8
    colors_uint8 = (rgbs * 255).astype(np.uint8)

    # --- ATTRIBUTE CONVERSION ---
    # Opacity: Inverse Sigmoid (Logit)
    # Clamp to avoid inf
    opacities = np.clip(opacities, 1e-6, 1 - 1e-6)
    logit_opacities = np.log(opacities / (1 - opacities)).flatten()
    ...
    elements['opacity'] = logit_opacities
    ...
    print(f"[SUCCESS] Saved Colored PLY to {output_path}")
@torch.no_grad()
def forward_mast3r(pairs, model, cache_path, desc_conf='desc_conf',
    ...
    for img1, img2 in tqdm(pairs):
        ...
        gaussians_path1 = cache_path + f'/gaussians/{idx1}/{idx2}.pth' # img1's Gaussians in pair (img1,img2)
        gaussians_path2 = cache_path + f'/gaussians/{idx2}/{idx1}.pth' # img2's Gaussians in pair (img2,img1)
        ...
        if not all(os.path.isfile(p) for p in (path1, path2, path_corres, gaussians_path1, gaussians_path2)):
            if model is None:
                continue
            ...
            # NEW: Extract Gaussian attributes from ALL 4 outputs
            # res[0] = res11 (img1 in frame of img1)
            # res[1] = res21 (img2 in frame of img1)
            # res[2] = res22 (img2 in frame of img2)
            # res[3] = res12 (img1 in frame of img2)
            
            gaussian_attrs = []
            for r in res:
                attrs = {
                    'sh': r['sh'][0],           # [H, W, K]
                    'scales': r['scales'][0],   # [H, W, 3]
                    'rotations': r['rotations'][0],  # [H, W, 4]
                    'opacities': r['opacities'][0],  # [H, W, 1]
                    'offsets': r.get('offsets', torch.zeros_like(r['pts3d'][0])),  # [H, W, 3]
                    'means': r.get('means', torch.zeros_like(r['pts3d'][0])),        # [H, W, 3]
                }
                gaussian_attrs.append(attrs)
            
            g11, g21, g22, g12 = gaussian_attrs
            ...
            gaussian_data1 = {
                ...
                'g2': to_cpu(g21),  # img2's Gaussians in img1's frame
            }
            ...
            gaussian_data2 = {
                ...
                'g2': to_cpu(g12),  # img1's Gaussians in img2's frame
            }
            torch.save(gaussian_data2, mkdir_for(gaussians_path2))
        ...
        res_paths[img1['instance'], img2['instance']] = (
            (path1, path2), 
             path_corres,
             (gaussians_path1, gaussians_path2)
        )
...
#     canonical_views = {}
#     canonical_gaussians = {}  # NEW Dictionary to store the final canonical gaussian params for every image
#     pairwise_scores = torch.zeros((len(imgs), len(imgs)), device=device)
#     canonical_paths = []
#     preds_21 = {}

#     for img in tqdm(imgs):
#         # [FIX 1] Initialize container for this image iteration
#         avg_gaussians = None 
#         canonical_gaussian = None
#         canon = None
#         focal = None
#         # [FIX 4] Initialize path variables to None to prevent UnboundLocalError
#         cache = None
#         gaussian_cache = None
#         if cache_path:
#             cache = os.path.join(cache_path, 'canon_views', hash_md5(img) + f'_{subsample=}_{kw=}.pth')
#             gaussian_cache = os.path.join(cache_path, 'canon_gaussians', hash_md5(img) + f'_{subsample=}.pth')
#             canonical_paths.append(cache)
#         try:
#             (canon, canon2, cconf), focal = torch.load(cache, map_location=device)
#             canonical_gaussian = torch.load(gaussian_cache, map_location=device)
#             # [FIX 2] If loaded successfully, set avg_gaussians immediately
#             avg_gaussians = canonical_gaussian
...
#         ptmaps11 = None
#         pixels = {}
#          #  [NEW] 1. Initialize storage for Gaussian parameters (stacks)
#         gaussian_stack = {
#             'sh': [],
#             'scales': [],
#             'rotations': [],
...
#         n = 0
#         # Iterate over all pairs to find predictions for the current 'img'
#         for (img1, img2), ((path1, path2), path_corres,(gpath1, gpath2)) in tmp_pairs.items():
#             print(f"[GPATH] gpath1={gpath1}, gpath2={gpath2}")
#             print(f"[DEBUG] canonical img={img} | pair=({img1}, {img2})")
#             score = None
#             if img == img1:
#                 print(f"[DEBUG] -> img matches img1; loading gpath1 = {gpath2}")
...
#                 # NEW: Load Gaussian attributes
#                 g_data = torch.load(gpath2, map_location=device)
#                 print("[DEBUG] Gaussian keys:", list(g_data.keys()))

#                 current_gaussians = g_data['g1']  # Gaussians for img1
...
#             if img == img2:
#                 print(f"[DEBUG] -> img matches img2; loading gpath2 = {gpath2}")
...
#                 g_data = torch.load(gpath2, map_location=device)
#                 print("[DEBUG] Gaussian keys:", list(g_data.keys()))
#                 current_gaussians = g_data['g1']  # Gaussians for img2
...
#                 # If we already have the canonical data from cache, skip accumulation
#                 if canon is not None and canonical_gaussian is not None:
#                     continue
#                 if ptmaps11 is None:
#                     H, W = C.shape
...
#                 ptmaps11[n] = X # Stacking Points
#                 confs11[n] = C # Stacking Confidences
#                 # [NEW] Stack Gaussian Attributes
#                 # We append the current gaussians to the list.
#                 for key in gaussian_stack:
#                     # current_gaussians[key] has shape (H, W, D)
#                     gaussian_stack[key].append(current_gaussians[key])

#                 n += 1
#         # Calculate Canonical Data if NOT loaded from cache
#         if canon is None:
#             # [Existing] Average the 3D points
#             canon, canon2, cconf = canonical_view(ptmaps11, confs11, subsample, **kw)
#             # [NEW] Average the Gaussian Attributes
#             # We use the accumulated confidence maps (confs11) to do a weighted average
#             avg_gaussians = {}
#             # Prepare confidence for broadcasting. 
#             # confs11 shape is (N_pairs, H, W). We need (N_pairs, H, W, 1)
#             weights = confs11.unsqueeze(-1)
#             sum_weights = weights.sum(dim=0) + 1e-6 # Avoid div by zero

#             for key in gaussian_stack:
#                 if len(gaussian_stack[key]) == 0: continue

#                 # Stack the list into a tensor: (N_pairs, H, W, D)
#                 stacked_param = torch.stack(gaussian_stack[key], dim=0)
                
#                 # Perform Weighted Average:
#                 # (Sum(Weight * Param)) / Sum(Weight)
#                 avg_param = (stacked_param * weights).sum(dim=0) / sum_weights
                
#                 avg_gaussians[key] = avg_param
#             # [NEW] Special Handling for Rotations (Quaternions)
#             # Averaging quaternions linearly is an approximation. 
#             # We MUST normalize them afterwards to ensure they are valid unit quaternions.
#             if 'rotations' in avg_gaussians:
#                 avg_gaussians['rotations'] = F.normalize(avg_gaussians['rotations'], dim=-1)
#             # Save to cache
#             if cache:
#                 torch.save(to_cpu(avg_gaussians), mkdir_for(gaussian_cache))
            
#             del ptmaps11
#             del confs11
#         # [FIX 3] Ensure avg_gaussians is assigned if we loaded from cache
#         elif avg_gaussians is None and canonical_gaussian is not None:
#              avg_gaussians = canonical_gaussian

#         # # NEW: Aggregate Gaussian attributes (simple averaging for now)
#         # if canonical_gaussian is None and gaussian_storage['sh']:
#         #     canonical_gaussian = {}
#         #     confs_expanded = confs11.unsqueeze(-1)  # [N, H, W, 1]
            
#         #     for key in gaussian_storage:
#         #         stacked = torch.stack(gaussian_storage[key], dim=0)  # [N, H, W, ...]
                
#         #         # Weighted average by confidence
#         #         canonical_gaussian[key] = (
#         #             (confs_expanded * stacked).sum(0) / confs_expanded.sum(0)
#         #         )
            
#         #     # Save to cache
#         #     if cache:
#         #         torch.save(to_cpu(canonical_gaussian), mkdir_for(gaussian_cache))
...
def prepare_canonical_data(imgs, tmp_pairs, subsample, order_imgs=False, min_conf_thr=0,
    ...
    for img in tqdm(imgs):
        # [FIX] Initialize containers
        avg_gaussians = None 
        canonical_gaussian = None
        canon = None
        focal = None
        
        # [FIX] Initialize path variables to None to avoid UnboundLocalError
        cache = None
        gaussian_cache = None
        ...
        try:
            if cache and os.path.exists(cache):
                (canon, canon2, cconf), focal = torch.load(cache, map_location=device)
            if gaussian_cache and os.path.exists(gaussian_cache):
                canonical_gaussian = torch.load(gaussian_cache, map_location=device)
            if canonical_gaussian is not None:
                avg_gaussians = canonical_gaussian
        except (IOError, FileNotFoundError, EOFError):
            canon = focal = None
            avg_gaussians = None

        # 2. Setup Running Average Accumulators (Memory Efficient)
        accumulator = {} 
        total_weight_map = None 
        ...
        for (img1, img2), ((path1, path2), path_corres, (gpath1, gpath2)) in tmp_pairs.items():
            ...
            if img == img1:
                ...
                g_data = torch.load(gpath1, map_location=device)
                current_gaussians = g_data['g1']
                
                if img not in preds_21: preds_21[img] = {}
                preds_21[img][img2] = X2[::subsample, ::subsample].reshape(-1, 3), C2[::subsample, ::subsample].ravel()
            ...
            if score is not None:
                ...
                # If cache hit, skip accumulation
                if canon is not None and avg_gaussians is not None:
                    continue
                ...
                # --- MEMORY OPTIMIZATION & SHAPE FIX ---
                # Weight shape: [H, W, 1]
                weight = C.unsqueeze(-1) 
                
                if total_weight_map is None:
                    total_weight_map = torch.zeros_like(weight)
                total_weight_map += weight

                for key, param in current_gaussians.items():
                    # print(f"[DEBUG] Processing Gaussian param key='{key}' with shape {param.shape} and weight shape {weight.shape}")
                    if param.ndim == 4 and param.shape[-1]==1: 
                        # param = param.squeeze(3)
                        # [H, W, C, 1] -> [H, W, C]
                        param = param.squeeze(-1)
                        
                    # # If param is [C, H, W] (3D) -> permute to [H, W, C]
                    # # We check if dim 0 is smaller than spatial dims (heuristic for channels)
                    # if param.ndim == 3 and param.shape[0] < param.shape[1]:
                    ...
                    if key not in accumulator:
                        accumulator[key] = weighted_param
                    else:
                        accumulator[key] += weighted_param
        ...
        # 4. Finalize Canonical Data
        if canon is None:
            canon, canon2, cconf = canonical_view(ptmaps11, confs11, subsample, **kw)
            # Cleanup
            del ptmaps11
            del confs11
        # --- B. Handle Gaussians (Splatt3R) ---
        # We calculate gaussians if they are missing AND we have accumulated data
        if avg_gaussians is None and accumulator:
        
            avg_gaussians = {}
            # Avoid division by zero
            denom = total_weight_map + 1e-6
            
            for key, summed_param in accumulator.items():
                if key in ['means', 'xyz', 'pos']:
                    continue
                avg_gaussians[key] = summed_param / denom
            
            # Normalize Rotations
            if 'rotations' in avg_gaussians:
                avg_gaussians['rotations'] = F.normalize(avg_gaussians['rotations'], dim=-1)
            # [NEW] Explicitly assign the robust canonical geometry as the Gaussian means
            # canon is typically [H, W, 3]
            avg_gaussians['means'] = canon
            # Save to cache
            if cache_path:
                torch.save(to_cpu(avg_gaussians), mkdir_for(gaussian_cache))
            # Cleanup gaussian memory
            del accumulator
            del total_weight_map
            torch.cuda.empty_cache()
        # --- C. Fallback Safety ---
        elif avg_gaussians is None and canonical_gaussian is not None:
             avg_gaussians = canonical_gaussian
